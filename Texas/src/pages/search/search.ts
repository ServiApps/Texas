...
@Component({
  template: `<ion-header>
<ion-toolbar color="shadow">
  <ion-title text-center>Filter</ion-title>
  <ion-buttons end>
      <button ion-button ligth class="has-icon icon-only has-badge" (click)="close()">
...
<div *ngIf="filters.item.length">
<div *ngFor="let item of filters.item; let i = index">

 <ion-item>{{item.name}}</ion-item>
   <div *ngIf="item.values.value.length">
        <ion-item *ngFor="let value of item.values.value; let j = index">

          <ion-label>{{value.label}} ({{value.count}})</ion-label>
          <ion-checkbox [(ngModel)]="filters.item[i].values.value[j].selected"></ion-checkbox>
        </ion-item>
    </div>    
    <ion-item *ngIf="!item.values.value.length">
      <ion-label>{{item.values.value.label}} ({{item.values.value.count}})</ion-label>
      <ion-checkbox [(ngModel)]="filters.item[i].values.value.selected"></ion-checkbox>
    </ion-item>
...
<div *ngIf="!filters.item.length">
 <ion-item>{{filters.item.name}}</ion-item>
   <div *ngIf="filters.item.values.value.length">
        <ion-item *ngFor="let value of filters.item.values.value;  let j = index">
          <ion-label>{{value.label}} ({{value.count}})</ion-label>
          <ion-checkbox [(ngModel)]="filters.item.values.value[j].selected"></ion-checkbox>
        </ion-item>
    </div>    
    <ion-item *ngIf="!filters.item.values.value.length">
      <ion-label>{{filters.item.values.value.label}} ({{filters.item.values.value.count}})</ion-label>
      <ion-checkbox [(ngModel)]="filters.item.values.value.selected"></ion-checkbox>
    </ion-item>
</div>
...
<ion-footer primary *ngIf="filters">

<ion-grid no-padding>
  <ion-row no-padding>
    <ion-col primary width-50 no-padding>
    <button ion-button full clear no-padding no-margin (click)="clearAll()">
      Clear
    </button>
    </ion-col>
       <ion-col royal width-50 no-padding>
    <button ion-button full clear color="shadow" no-padding no-margin (click)="apply()">
      Apply
    </button>
    </ion-col>
  </ion-row>
